...
| Currently integrated upstream version | **1.6.0** (released 2026-05-01 by upstream) — current built tag `v1.6.0-mine.9` |
...
| `app/api/auth/sso/start-redirect/route.ts` | mine.8: NEW server-side GET-302 OIDC initiator. Mirrors the existing POST `/api/auth/sso/start` handler — same PKCE mint, same `sso_pending` cookie, same X-Forwarded-Host fallback — but answers a browser GET with a direct 302 to the IdP's authorize URL instead of returning JSON. Used by the binarybeachio auth-bridge's bulwark adapter so the user is sent straight from the bridge to Zitadel without ever rendering the React `/<locale>/login` page. Replaces the prior auto-SSO `useEffect` approach, which had a `sso_attempted` sessionStorage flag whose 30s setTimeout cleanup was killed by the redirect-to-Zitadel page unload, leaving the flag stuck and forcing the user to click the "Sign in with SSO" button manually on every subsequent visit. | ~120 | None — net-new file, sibling to `/api/auth/sso/start/route.ts`. |
| `lib/auth/bb-edge-identity.ts` | mine.9: NEW shared helper exporting the `_bb_edge_sub` cookie name, the `X-Auth-Request-User` header name, and the cookie-options object used by both the OIDC-callback set-site and the root middleware check-site. Per docs/conventions/per-app-edge-identity-validation.md. | ~15 | None — net-new file. |
| `proxy.ts` | mine.9: prepend `enforceEdgeIdentity()` to the existing Next 16 proxy (renamed from `middleware.ts` upstream). Compares `_bb_edge_sub` cookie to `X-Auth-Request-User` header on every authenticated request, **including `/api/*` state-bearing routes** (the existing `PROXY_SKIP_PATTERN` short-circuits API routes from the CSP/intl logic but the edge check runs FIRST, before that short-circuit, so it can guard `/api/auth/token` PUT/DELETE and `/api/settings/*` etc. — the routes where a stale Bulwark session could still rotate tokens or read state). Mismatch → clear `_bb_edge_sub` + every `jmap_rt_<slot>` cookie + 302 to `/`. Missing cookie → lazy-populate. Missing header → pass through (mobile/dev/break-glass). Skip-paths enumerated in `EDGE_SKIP_SEGMENT_PREFIXES` + `EDGE_LOCALE_PRE_AUTH_RE`: `/_next`, `/api/health`, `/api/config`, all OIDC-handshake routes (`/api/auth/sso/start*`, `/api/auth/sso/complete`, `/api/auth/token`), `/api/auth/session`, `/api/admin`, `/admin`, `/jmap-proxy`, `/api/auth/stalwart-context`, `/<locale>?/login`, `/<locale>?/auth/callback`, plus static asset paths. | ~95 | Low — additive prepend at the top of upstream's `proxy()` async function; the CSP/intl/locale handling that follows is untouched. |
| `app/api/auth/token/route.ts` | mine.9: in the POST handler (OAuth code-exchange path that mints `jmap_rt_<slot>`), set `_bb_edge_sub` cookie to the value of `X-Auth-Request-User` header on success. Three-line additive insertion immediately after the existing refresh-token-cookie write. | ~6 | Low — additive, gated on `request.headers.get('x-auth-request-user')` being non-empty. |
| `app/api/auth/sso/complete/route.ts` | mine.9: same as `/api/auth/token` — pin `_bb_edge_sub` after successful token exchange in the server-side SSO flow. Mirrors the additive shape so both OAuth flows reach the same end-state. | ~5 | Low — additive. |
| `lib/admin/types.ts` | mine.9: add `bridgeLogoutUrl` (env `BRIDGE_LOGOUT_URL`) to `CONFIG_ENV_MAP`. Used by `redirectToLogin()` in `stores/auth-store.ts` to send full-logout / session-expired flows through the platform bridge instead of the local `/login` page. Inert when env unset. | ~2 | Low — record-literal entry. |
| `app/api/config/route.ts` | mine.9: emit `bridgeLogoutUrl` on the `/api/config` JSON. | ~1 | Low — one new field. |
| `hooks/use-config.ts` | mine.9: add `bridgeLogoutUrl: string` to `ConfigData` and to all three `setConfig({...})` initializers in `useConfig()`. Also mirrors `configCache` onto `window.__bb_config_cache` after fetch so the auth-store can read it synchronously during the (synchronous) logout flow without an async dance or a circular import. | ~6 | Low — boilerplate + a 4-line `window`-mirror block in `fetchConfig`. |
| `stores/auth-store.ts` | mine.9: in `redirectToLogin()`, when `window.__bb_config_cache.bridgeLogoutUrl` is non-empty, navigate there instead of the local `/<locale>/login` path. Inert when unset. Single early-return added to an existing 5-line function. | ~10 | Low — an early-return guarded by a clearly-namespaced cache field. |
...
| `v1.6.0-mine.7` | superseded | Add `DISABLE_ADD_ACCOUNT` env-gated UI suppression. When `DISABLE_ADD_ACCOUNT=true`, the per-Bulwark "+ Add Account" button in `components/layout/account-switcher.tsx` is hidden. **Why**: the binarybeachio deployment puts oauth2-proxy in front of Bulwark with one Zitadel session per browser at `.binarybeach.io`. Adding a second Zitadel identity inside Bulwark fights that single-session model — the secondary account's bearer token authenticates against Stalwart fine, but the *edge* gate validates only the primary identity, so a deactivated primary kills access to all secondary accounts. Multi-identity moves to the platform layer (a "Switch BinaryBeach.io account" affordance that calls `/oauth2/sign_out` then `/oauth2/start?prompt=select_account` — see binarybeachio docs for the bridge `/switch` route). **Plumbing**: env → `CONFIG_ENV_MAP` (`lib/admin/types.ts`) → `/api/config` JSON (`app/api/config/route.ts`) → `useConfig()` hook (`hooks/use-config.ts`) → `account-switcher.tsx` consumer. Four files, ~12 lines net. Inert when env unset (defaults to `false` = upstream behavior). Direct nav to `/login?mode=add-account` is *not* blocked — operator-side workaround if it's ever needed; the dropdown is the only normal-path entry. |
| `v1.6.0-mine.8` | superseded | Add `app/api/auth/sso/start-redirect/route.ts` — a server-side GET-302 OIDC initiator. Mirrors the existing POST `/api/auth/sso/start` handler (PKCE mint, `sso_pending` cookie, X-Forwarded-Host fallback for `requestOrigin`) but answers a browser GET with a direct 302 to the Zitadel authorize URL instead of returning JSON. **Why**: the binarybeachio auth-bridge's bulwark adapter (`infrastructure/auth-bridge/src/adapters/bulwark.ts`) needs a single GET hop from the bridge to the IdP — the prior approach (auto-SSO `useEffect` on `/login`) was inconsistent because its `sso_attempted` sessionStorage loop-guard's 30s setTimeout cleanup was killed by the page navigating to Zitadel before it could fire, leaving the flag stuck and forcing the user to manually click "Sign in with SSO" on every subsequent visit. With this endpoint and the matching Traefik regex middleware in `infrastructure/bulwark/docker-compose.yml`, the React `/<locale>/login` page is never rendered. **Behavior**: query param `locale` (constrained to `^[a-z]{2}(-[A-Z]{2})?$`, defaults to `en`) controls both `ui_locales=` on the authorize URL and the segment on the redirect_uri (which Zitadel must have whitelisted at `https://mail.binarybeach.io/<locale>/auth/callback`). Standalone — no upstream files modified. |
| `v1.6.0-mine.9` | active | Per-app edge-identity validation per `docs/conventions/per-app-edge-identity-validation.md`, plus a bundled BRIDGE_LOGOUT_URL knob that re-points the SPA's existing sign-out button at the platform `/logout` route. **Why edge-identity validation**: when `bridge.binarybeach.io/logout` clears the oauth2-proxy edge cookie and a different Zitadel identity signs back in, Bulwark's per-account `jmap_rt_<slot>` cookies survive the swap — leaving identity A's mailbox visible on a refresh even though the edge has rebound to identity B (bug repro 2026-05-04 in Mattermost; structurally identical for Bulwark). **The patch**: (1) NEW `lib/auth/bb-edge-identity.ts` defines the `_bb_edge_sub` cookie name, header name, and cookie options. (2) `proxy.ts` (Next 16's renamed-from-`middleware` proxy) gains a `enforceEdgeIdentity()` prepend that runs on every request including `/api/*` state-bearing routes; mismatch clears `_bb_edge_sub` + all `jmap_rt_<slot>` cookies + 302s to `/`. Skip-paths cover OIDC-handshake routes, healthcheck, admin, JMAP-proxy, locale-prefixed login + callback, and static assets. (3) `app/api/auth/token/route.ts` POST + `app/api/auth/sso/complete/route.ts` POST now set `_bb_edge_sub` to the `X-Auth-Request-User` header value on a successful token exchange. **Why bundled BRIDGE_LOGOUT_URL**: per the 2026-05-04 session debrief at `docs/services/auth-bridge/session-debrief-2026-05-04-edge-validation-and-logout.md`, every platform-edge-gated app's existing sign-out button should terminate at the bridge's `/logout` so oauth2-proxy clears `_bb_oauth2` AND back-channels Zitadel's `end_session` in one hop. **The plumbing**: env → `CONFIG_ENV_MAP` → `/api/config` → `useConfig()` ConfigData + `window.__bb_config_cache` (mirrored synchronous read for the auth-store) → an early-return in `redirectToLogin()`. Inert when unset. **Why this isn't a sibling `middleware.ts`**: Next 16 forbids both `middleware.ts` and `proxy.ts` in the same project — the upstream renamed to `proxy.ts` already; we merge into it rather than ship a redundant file. Trade-off: edge check runs on every request — the `shouldSkipEdgeCheck()` early-return is a tight loop over ~17 prefixes, no measurable perf impact at our scale. |
