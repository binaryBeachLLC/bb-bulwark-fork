...
Thirteen files. Total: ~330 lines.
...
| `middleware.ts` | mine.9: NEW Next.js root middleware. Implements per-app edge-identity validation: compares `_bb_edge_sub` cookie to `X-Auth-Request-User` header on every authenticated request. Mismatch → clear `_bb_edge_sub` + every `jmap_rt_<slot>` cookie + 302 to `/`. Missing cookie → lazy-populate. Missing header → pass through (mobile/dev/break-glass). Skip-paths enumerated in `SKIP_SEGMENT_PREFIXES` + `LOCALE_PRE_AUTH_RE`: `/_next`, `/api/health`, `/api/config`, all OIDC-handshake routes (`/api/auth/sso/start*`, `/api/auth/sso/complete`, `/api/auth/token`), `/api/auth/session`, `/api/admin`, `/admin`, `/jmap-proxy`, `/api/auth/stalwart-context`, `/<locale>?/login`, `/<locale>?/auth/callback`, plus static asset paths. | ~95 | None — net-new file at the project root. Next.js auto-detects `middleware.ts`. |
