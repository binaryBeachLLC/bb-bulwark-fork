...
export default function LoginPage() {
  ...
  const handleOAuthLogin = async () => {
    ...
    // binarybeachio (mine.6): persist the next free cookie slot so the auth
    // callback (loginWithOAuth in stores/auth-store.ts) writes the refresh
    // token to the correct `jmap_rt_<slot>` cookie. Upstream reads
    // `oauth_cookie_slot` but never sets it, so every account collapsed onto
    // slot 0 and clobbered each other's refresh tokens. getNextCookieSlot()
    // returns 0 when no accounts exist (correct for first sign-in) and the
    // lowest unused slot otherwise (correct for "+ Add Account").
