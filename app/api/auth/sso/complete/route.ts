...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // binarybeachio (mine.6): accept `slot` from request body so multi-account
    // sign-in can write the refresh token to the correct jmap_rt_<slot> cookie.
    // Upstream hardcoded slot 0, so the "+ Add Account" flow overwrote the
    // first account's refresh-token cookie. Default to 0 for back-compat with
    // any caller that omits slot. Mirrors the validation in /api/auth/token POST.
